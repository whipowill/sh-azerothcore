...
-- found a copy of the ItemExtendedCost table here:
-- https://raw.githubusercontent.com/Kaev/AzerothcoreDBCToSQL/refs/heads/master/ItemExtendedCost.sql

-- Going to try and make all honor items 4x as expensive.
-- the issue is honor costs are hardcoded, so you can't just change them
-- you have to find all the references and patch the references

-- Classic Legacy PVP Vendors to edit:
-- 12784, -- Lieutenant Jackspring <Legacy Weapon Quartermaster>
-- 12785, -- Sergeant Major Clate <Legacy Armor Quartermaster>
-- 12794, -- Stone Guard Zarg <Legacy Weapon Quartermaster>
-- 12795, -- First Sergeant Hola'mahi <Legacy Armor Quartermaster>
...
SET nv.ExtendedCost = 2427
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
AND nv.ExtendedCost = 542
AND it.Quality = 4;

-- 463 (15k) to 2569 (60k)
UPDATE npc_vendor nv JOIN item_template it ON nv.item = it.entry
SET nv.ExtendedCost = 2569
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
AND nv.ExtendedCost = 463
AND it.Quality = 4;
...
SET nv.ExtendedCost = 2569
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
AND nv.ExtendedCost = 464
AND it.Quality = 4;

-- 2257 (25k) to 2570 (50k)
UPDATE npc_vendor nv JOIN item_template it ON nv.item = it.entry
SET nv.ExtendedCost = 2570
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
...
SET nv.ExtendedCost = 2427
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
AND nv.ExtendedCost = 465
AND it.Quality = 4;

-- 444 (5k) to 2254 (20k)
UPDATE npc_vendor nv JOIN item_template it ON nv.item = it.entry
SET nv.ExtendedCost = 2254
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
AND nv.ExtendedCost = 444
AND it.Quality = 4;

-- 427 (3k) to 2248 (12k)
UPDATE npc_vendor nv JOIN item_template it ON nv.item = it.entry
SET nv.ExtendedCost = 2248
WHERE FIND_IN_SET(nv.entry, @vendor_entries) > 0
AND nv.ExtendedCost = 427
AND it.Quality = 4;
